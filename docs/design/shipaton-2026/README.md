...
関所は、スマートフォンの利用制限を「罰」ではなく、3Dの番人と一緒に守る静かな約束として体験させる。個人利用だけでも成立させたうえで、ソーシャルMVPとして8〜20人程度を毎週自動編成する匿名週間リーグを追加する。個別メッセージ、グループチャット、自由入力プロフィール、罰金機能は含めない。
...
価格方針は月額790円、年額3,990円、7日間無料。Turningと同様に月額を基準価格として見せ、年額は月額12か月分に対して約58%割引とする。App Store Connectの価格はリリース前に別途更新する。
...
Shipaton後は、約束を守った日数や継続によって番人が育つ仕組みを追加する。育成そのものは無料とし、追加キャラクター、衣装、部屋の装飾、定額表示のアイテムパックを非消耗型IAPとして販売する。ランダムなガチャ、制限解除アイテム、育成を実質的に購入必須にする設計は採用しない。
